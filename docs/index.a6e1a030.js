...
function makeId() {
  ...
  for (let i = 0; i < 40; i++) result.push(characters[Math.floor(Math.random() * characters.length)]);
  return result.join('');
}
...
const connectedTrackers = new Set();
...
async function joinRoom(roomId) {
  ...
  if (cachedName == null) name = prompt('Your name is...'); else name = prompt('Your name is...', cachedName);
  if (name == null || name === '') name = `Peer ${peerId.slice(0, 8)}`;
  localStorage.setItem('editor-name', name);
  updatePeersDisplay();
  const Tracker = await require('bittorrent-tracker');
  tracker = new Tracker({
  ...
  tracker.setInterval(15 * 60 * 1000);
  // every 15 minutes
  ...
  tracker.on('error', e => {
    ...
    if (connectedTrackers.size === 0) document.querySelector('#connection-status').textContent = 'Connecting...';
  });
...
function setupConnection(peer) {
  ...
  if (existingValue.length > 0) peer.send(JSON.stringify({
    type: 'state',
    value: existingValue,
    language: languagesSelect.value
  }));
  const cursorPosition = editor?.getPosition?.();
  if (cursorPosition != null) peer.send(JSON.stringify({
    type: 'cursor',
...
async function updatePeerCursor(peer) {
  const monaco = await require('monaco-editor');
...
async function main() {
  ...
  languagesSelect.innerHTML = monaco.languages.getLanguages().filter(lang => lang.aliases !== undefined).map(lang => {
...
},{"buffer":"3susO","bittorrent-tracker":"4EaSb","monaco-editor":"2Gc0r"}],"3susO":[function(require,module,exports) {
/*!
...
},{}],"4EaSb":[function(require,module,exports) {
module.exports = (require("./loaders/browser/js-loader")(require('./bundle-url').getBundleURL() + "bittorrent-tracker.e1b6a29c.js").catch(err => {delete module.bundle.cache[module.id]; throw err;})).then(() => module.bundle.root('1CQbf'));
...
},{}],"2Gc0r":[function(require,module,exports) {
module.exports = Promise.all([require("./loaders/browser/css-loader")(require('./bundle-url').getBundleURL() + "editor.main.75264053.css").catch(err => {delete module.bundle.cache[module.id]; throw err;}), require("./loaders/browser/js-loader")(require('./bundle-url').getBundleURL() + "editor.main.2bb1a556.js").catch(err => {delete module.bundle.cache[module.id]; throw err;})]).then(() => module.bundle.root('2xBsR'));
},{"./loaders/browser/css-loader":"qsgYX","./bundle-url":"3seVR","./loaders/browser/js-loader":"2Ucmd"}],"qsgYX":[function(require,module,exports) {
const cacheLoader = require('../../cacheLoader');
...
},{"../../cacheLoader":"4zpnH"}]},["69fNU","3rfh7"], "3rfh7", "parcelRequire1b1f")
